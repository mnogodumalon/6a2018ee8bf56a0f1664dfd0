...
import type { Repositories, Issues, Kommentare, CreateRepositories, CreateIssues, CreateKommentare } from '@/types/app';
...
export class LivingAppsService {
  ...
  static async getRepositories(): Promise<Repositories[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.REPOSITORIES}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Repositories[];
    return enrichLookupFields(records, 'repositories');
  }
  static async getRepositorie(id: string): Promise<Repositories | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.REPOSITORIES}/records/${id}`);
    const record = { record_id: data.id, ...data } as Repositories;
    return enrichLookupFields([record], 'repositories')[0];
  }
  static async createRepositorie(fields: CreateRepositories) {
    return callApi('POST', `/apps/${APP_IDS.REPOSITORIES}/records`, { fields: cleanFieldsForApi(fields as any, 'repositories') });
  }
  static async updateRepositorie(id: string, fields: Partial<CreateRepositories>) {
    return callApi('PATCH', `/apps/${APP_IDS.REPOSITORIES}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'repositories') });
  }
  static async deleteRepositorie(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.REPOSITORIES}/records/${id}`);
  }

  // --- ISSUES ---
  static async getIssues(): Promise<Issues[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.ISSUES}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Issues[];
    return enrichLookupFields(records, 'issues');
  }
  static async getIssue(id: string): Promise<Issues | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.ISSUES}/records/${id}`);
    const record = { record_id: data.id, ...data } as Issues;
    return enrichLookupFields([record], 'issues')[0];
  }
  static async createIssue(fields: CreateIssues) {
    return callApi('POST', `/apps/${APP_IDS.ISSUES}/records`, { fields: cleanFieldsForApi(fields as any, 'issues') });
  }
  static async updateIssue(id: string, fields: Partial<CreateIssues>) {
    return callApi('PATCH', `/apps/${APP_IDS.ISSUES}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'issues') });
  }
  static async deleteIssue(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.ISSUES}/records/${id}`);
  }

  // --- KOMMENTARE ---
  static async getKommentare(): Promise<Kommentare[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.KOMMENTARE}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Kommentare[];
    return enrichLookupFields(records, 'kommentare');
  }
  static async getKommentareEntry(id: string): Promise<Kommentare | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.KOMMENTARE}/records/${id}`);
    const record = { record_id: data.id, ...data } as Kommentare;
    return enrichLookupFields([record], 'kommentare')[0];
  }
  static async createKommentareEntry(fields: CreateKommentare) {
    return callApi('POST', `/apps/${APP_IDS.KOMMENTARE}/records`, { fields: cleanFieldsForApi(fields as any, 'kommentare') });
  }
  static async updateKommentareEntry(id: string, fields: Partial<CreateKommentare>) {
    return callApi('PATCH', `/apps/${APP_IDS.KOMMENTARE}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'kommentare') });
  }
  static async deleteKommentareEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.KOMMENTARE}/records/${id}`);
  }
